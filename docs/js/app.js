...
var M={
  ...
  activity:[
    {icon:"Γ£à",color:"green", title:"Volkswagen Virtus GT sold",          sub:"KM-2024-008 ┬╖ Γé╣16.25L",          time:"2m ago"},
    {icon:"≡ƒÜù",color:"orange",title:"MG Hector Plus added via AI intake", sub:"KM-2024-009 ┬╖ Pending review",   time:"38m ago"},
    {icon:"≡ƒô₧",color:"blue",  title:"New lead: Deepak Nair",              sub:"Interested in Virtus / Vento",   time:"1h ago"},
    {icon:"≡ƒöÆ",color:"purple",title:"Scorpio N reserved",                 sub:"KM-2024-007 ┬╖ Vikram Reddy",     time:"3h ago"},
    {icon:"≡ƒÆ¼",color:"green", title:"WhatsApp inquiry ΓÇö Nexon EV",        sub:"91-981-234-5678 ┬╖ Priya Sharma", time:"4h ago"},
    {icon:"ΓÜá∩╕Å",color:"red",  title:"Conflict detected on RC scan",        sub:"KM-2024-009 ┬╖ Reg mismatch",     time:"5h ago"}
  ],
...
function money(v){if(v==null)return"ΓÇö";var n=Number(v);if(isNaN(n))return v;if(n>=10000000)return"Γé╣"+(n/10000000).toFixed(2)+"Cr";if(n>=100000)return"Γé╣"+(n/100000).toFixed(2)+"L";return"Γé╣"+n.toLocaleString("en-IN");}
function km(v){if(v==null)return"ΓÇö";var n=Number(v);return n>=1000?(n/1000).toFixed(1)+"k km":n.toLocaleString("en-IN")+" km";}
...
function toast(msg,type,title){
  ...
  el.innerHTML='<span class="toast-icon">'+icons[type]+'</span><div class="toast-content"><div class="toast-title">'+esc(title||titles[type])+'</div><div class="toast-msg">'+esc(msg)+'</div></div><button class="toast-dismiss" aria-label="Dismiss">Γ£ò</button>';
...
function renderDashboard(){
  setContent('<div class="loading-state"><div class="spinner"></div><span>LoadingΓÇª</span></div>');
  var a=M.analytics;
  var sb=a.status_breakdown;
  var total=a.total_vehicles||1;

  var kpis=[
    {label:"Total Inventory",  val:num(a.total_vehicles), sub:"All vehicles",       icon:"≡ƒÜù", color:"orange"},
    {label:"For Sale",         val:num(a.active_for_sale),sub:"Active listings",    icon:"Γ£à", color:"green"},
    {label:"Sold",             val:num(a.sold),           sub:"All time",           icon:"≡ƒÆ░", color:"blue"},
    {label:"Customers",        val:num(a.total_customers),sub:"In CRM",             icon:"≡ƒæÑ", color:"purple"},
    {label:"Revenue MTD",      val:money(a.revenue_mtd),  sub:"This month",         icon:"≡ƒôê", color:"orange"},
    {label:"Open Handoffs",    val:num(a.open_handoffs),  sub:"Needs attention",    icon:"ΓÜá∩╕Å", color:"red"}
  ];
  ...
  setContent(
    ...
    +'<div class="page-header-actions"><button class="btn btn-ghost btn-sm" onclick="navigate(\'analytics\')">≡ƒôè Full Report</button></div></div>'
    ...
    +'<div class="bottom-row">'
      ...
      +'<button class="btn btn-ghost btn-sm" onclick="navigate(\'inventory\')">View all ΓåÆ</button></div>'
...
function buildRevenueChart(){
  ...
  CHARTS.revenue=new Chart(c,{
    ...
    data:{labels:["Sep","Oct","Nov","Dec","Jan","Feb","Mar","Apr","May","Jun","Jul","Aug"],
      datasets:[
        {label:"Revenue (Γé╣L)",data:M.revenue,borderColor:"#f97316",backgroundColor:"rgba(249,115,22,.12)",borderWidth:2.5,tension:.4,fill:true,pointBackgroundColor:"#f97316",pointRadius:4,pointHoverRadius:6},
    ...
    options:{maintainAspectRatio:false,interaction:{mode:"index",intersect:false},
      ...
      scales:{
        ...
        y:{grid:{color:d.grid},ticks:{color:d.text,font:{size:11},callback:function(v){return"Γé╣"+v+"L";}},position:"left"},
...
/* ΓòÉΓòÉ INVENTORY ΓòÉΓòÉ */
function renderInventory(){
  setContent(
    '<div class="page-header"><div class="page-header-text"><h1>Inventory</h1><p>All vehicles in stock.</p></div>'
    +'<div class="page-header-actions">'
    +'<button class="btn btn-ghost btn-sm" id="exportBtn">Γ¼ç Export</button>'
    +'<button class="btn btn-primary btn-sm" onclick="navigate(\'intake\')">∩╝ï Add Vehicle</button>'
    +'</div></div>'
    +'<div class="toolbar">'
      +'<div class="toolbar-left">'
        ...
        +'<input type="text" id="invQ" placeholder="Search make, model, stock IDΓÇª" value="'+esc(S.q)+'"></div>'
      ...
      +'<div class="toolbar-right"><div class="view-toggle">'
        +'<button class="view-btn'+(S.mode==="grid"?" active":"")+ '" id="vgrid" title="Grid">Γè₧</button>'
        +'<button class="view-btn'+(S.mode==="list"?" active":"")+ '" id="vlist" title="List">Γÿ░</button>'
      +'</div></div>'
    +'</div>'
    +'<div id="invBody"></div>'
  );
...
function paintInv(body,cars){
  if(!cars.length){body.innerHTML='<div class="empty-state"><div class="empty-icon">≡ƒÜù</div><h3>No vehicles found</h3><p>Try a different filter.</p></div>';return;}
...
function carCard(v){
  ...
  return '<div class="car-card">'
    ...
    +(photos.length>1?'<div class="photo-count">≡ƒô╖ '+photos.length+'</div>':'')
    +'</div>'
    +'<div class="car-body">'
      ...
      +'<div class="car-footer">'
        +'<button class="btn btn-ghost btn-sm flex-1" onclick="showVehicle(\''+esc(v.vehicle_id)+'\')">≡ƒæü Details</button>'
        +(v.status!=="SOLD"&&v.status!=="ARCHIVED"
          ?'<select class="input status-select" onchange="setStatus(\''+esc(v.vehicle_id)+'\',this.value)">'
            +'<option value="">StatusΓÇª</option>'
...
function si(k,v){return'<div class="spec-item"><div class="spec-key">'+esc(k)+'</div><div class="spec-val">'+esc(v==null?"ΓÇö":v)+'</div></div>';}
...
window.setStatus=function(id,status){
  if(!status)return;
  POST(API+"/vehicles/"+id+"/status",{status:status,reason:"admin-ui"}).then(function(){toast("Status ΓåÆ "+status.replace(/_/g," "),"success");loadInv();}).catch(function(){var v=(S.vehicles||M.vehicles).find(function(x){return x.vehicle_id===id;});if(v)v.status=status;toast("Status ΓåÆ "+status.replace(/_/g," "),"success");loadInv();});
};

/* ΓòÉΓòÉ INTAKE ΓòÉΓòÉ */
function renderIntake(){
  setContent(
    '<div class="page-header"><div class="page-header-text"><h1>AI Vehicle Intake</h1><p>Drop photos or an RC image ΓÇö AI extracts all details automatically.</p></div></div>'
    +'<div class="grid-2" style="align-items:start">'
      +'<div style="display:flex;flex-direction:column;gap:20px">'
        +'<div class="card card-pad">'
          +'<div class="form-section-title" style="display:flex;align-items:center;gap:8px;font-size:12px;font-weight:700;color:var(--text-muted);text-transform:uppercase;letter-spacing:.6px;margin-bottom:16px;padding-bottom:10px;border-bottom:1px solid var(--border)">≡ƒô╖ Photos & Documents</div>'
          +'<div class="dropzone" id="dz"><div class="dropzone-icon">Γ¼å</div><h3>Drag & drop files here</h3><p>or <strong>browse</strong> to choose</p><p style="margin-top:6px;font-size:12px">RC, insurance, PUC, car photos ┬╖ Max 15 files</p></div>'
          +'<input type="file" id="fileInput" multiple accept="image/*,.pdf" hidden>'
          +'<div class="file-list" id="fileList"></div>'
        +'</div>'
        +'<div class="card card-pad">'
          +'<div class="form-section-title" style="display:flex;align-items:center;gap:8px;font-size:12px;font-weight:700;color:var(--text-muted);text-transform:uppercase;letter-spacing:.6px;margin-bottom:16px;padding-bottom:10px;border-bottom:1px solid var(--border)">Γä╣∩╕Å Seller Details (optional)</div>'
          +'<div class="form-grid">'
      ...
      +'<div style="display:flex;flex-direction:column;gap:20px">'
        +'<div class="card card-pad">'
          +'<div class="form-section-title" style="display:flex;align-items:center;gap:8px;font-size:12px;font-weight:700;color:var(--text-muted);text-transform:uppercase;letter-spacing:.6px;margin-bottom:16px;padding-bottom:10px;border-bottom:1px solid var(--border)">ΓÜí AI Options</div>'
          +'<div style="display:flex;flex-direction:column;gap:16px">'
            +'<div class="field"><label>Enhance Photos</label><select class="input" id="fProc"><option value="true">Yes ΓÇö generate web/social/thumbnail variants</option><option value="false">No ΓÇö store originals only</option></select></div>'
        ...
        +'</div>'
        +'<button class="btn btn-primary btn-lg w-full" id="intakeBtn" style="justify-content:center">ΓÜí Run AI Intake</button>'
        +'<div id="progWrap" style="display:none;margin-top:16px">'
          +'<div style="display:flex;justify-content:space-between;font-size:13px;font-weight:600;margin-bottom:6px"><span id="progLabel">ProcessingΓÇª</span><span id="progPct">0%</span></div>'
...
function addFiles(list){
  Array.from(list).forEach(function(f){
    ...
    li.innerHTML='<div class="file-item-icon">'+(f.type.startsWith("image")?"≡ƒû╝":"≡ƒôä")+'</div>'
      +'<div class="file-item-name">'+esc(f.name)+'</div>'
      +'<div class="file-item-size">'+(f.size/1024).toFixed(1)+' KB</div>'
      +'<button class="file-item-rm" aria-label="Remove">Γ£ò</button>';
...
function runIntake(){
  ...
  btn.disabled=true;btn.innerHTML="ΓÜí ProcessingΓÇª";
  if(pw)pw.style.display="block";
  var pct=0,si=0,steps=["UploadingΓÇª","Reading RCΓÇª","Analysing photosΓÇª","Generating variantsΓÇª","Saving to DBΓÇª"];
...
function doneIntake(ok,msg){
  ...
  setTimeout(function(){
    var btn=$("#intakeBtn"),pw=$("#progWrap");
    if(btn){btn.disabled=false;btn.innerHTML="ΓÜí Run AI Intake";}
...
function renderCustomers(){
  ...
  setContent(
    ...
    +'<div class="page-header-actions"><button class="btn btn-ghost btn-sm">Γ¼ç Export CSV</button></div></div>'
    +'<div class="card">'
      +'<div class="card-header"><span class="card-title"><span class="card-title-dot"></span>All Customers</span>'
        +'<div class="card-actions"><div class="search-box" style="max-width:220px"><svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><circle cx="11" cy="11" r="8"/><path d="m21 21-4.35-4.35"/></svg><input type="text" id="custQ" placeholder="SearchΓÇª"></div></div>'
      +'</div>'
...
function custRows(rows){
  ...
  return rows.map(function(c){
    return'<tr>'
      +'<td><div class="avatar-row"><div class="avatar">'+esc(ini(c.name))+'</div><div class="avatar-name">'+esc(c.name||"ΓÇö")+'</div></div></td>'
      +'<td class="font-mono" style="font-size:12.5px">'+esc(c.whatsapp_number||"ΓÇö")+'</td>'
      +'<td>'+esc(c.city||"ΓÇö")+'</td>'
      +'<td>'+bdg(c.lead_status)+'</td>'
      +'<td class="text-2">'+esc(c.preferred_vehicle||"ΓÇö")+'</td>'
      +'<td>'+(c.opt_out?'<span class="badge badge-CLOSED_LOST">Opted Out</span>':'<span class="badge badge-AVAILABLE">Opted In</span>')+'</td>'
      +'<td class="text-muted" style="font-size:12.5px">'+esc(c.created_at||"ΓÇö")+'</td>'
    +'</tr>';
  }).join("");
}
...
function renderLeads(){
  ...
  setContent(
    ...
    +(S.customers||M.customers).filter(function(c){return c.lead_status!=="CLOSED_LOST";}).map(function(c){
      return'<tr><td><div class="avatar-row"><div class="avatar">'+esc(ini(c.name))+'</div><div class="avatar-name">'+esc(c.name||"ΓÇö")+'</div></div></td>'
        +'<td class="text-2">'+esc(c.preferred_vehicle||"ΓÇö")+'</td>'
        +'<td>'+bdg(c.lead_status)+'</td>'
        +'<td class="font-mono" style="font-size:12.5px">'+esc((c.whatsapp_number||"").replace("91",""))+'</td>'
        +'<td><button class="btn btn-ghost btn-sm">≡ƒô₧ Follow Up</button></td></tr>';
    }).join("")
    +'</tbody></table></div></div>'
  );
...
/* ΓòÉΓòÉ ANALYTICS ΓòÉΓòÉ */
function renderAnalytics(){
  setContent('<div class="loading-state"><div class="spinner"></div><span>Building reportΓÇª</span></div>');
  var a=M.analytics;
  ...
  var kpis=[
    {label:"Total Vehicles",val:num(a.total_vehicles),sub:"All stock",icon:"≡ƒÜù",color:"orange"},
    {label:"Active Listings",val:num(a.active_for_sale),sub:"Ready to sell",icon:"Γ£à",color:"green"},
    {label:"Sold",val:num(a.sold),sub:"All time",icon:"≡ƒÆ░",color:"blue"},
    {label:"Avg Days to Sell",val:num(a.avg_days_to_sell||11),sub:"Per vehicle",icon:"≡ƒôà",color:"purple"},
    {label:"Revenue MTD",val:money(a.revenue_mtd),sub:"This month",icon:"≡ƒôê",color:"orange"},
    {label:"Customers",val:num(a.total_customers),sub:"In CRM",icon:"≡ƒæÑ",color:"green"}
  ];
  ...
  setContent(
    ...
    +'<div class="page-header-actions"><button class="btn btn-ghost btn-sm">Γ¼ç Export Report</button></div></div>'
...
function renderDocs(){
  ...
  setContent(
    ...
    +'<div class="docs-grid mb-24">'
      +'<a class="docs-card" href="/docs" target="_blank"><div class="docs-card-icon orange" style="font-size:22px">ΓÜí</div><h3>Swagger UI</h3><p>Interactive API explorer ΓÇö try every endpoint live.</p><div class="arrow">Open ΓåÆ</div></a>'
      +'<a class="docs-card" href="/redoc" target="_blank"><div class="docs-card-icon blue" style="font-size:22px">≡ƒôû</div><h3>ReDoc</h3><p>Clean structured reference documentation.</p><div class="arrow">Open ΓåÆ</div></a>'
      +'<a class="docs-card" href="/openapi.json" target="_blank"><div class="docs-card-icon green" style="font-size:22px">{ }</div><h3>OpenAPI JSON</h3><p>Import into Postman or any API client.</p><div class="arrow">Open ΓåÆ</div></a>'
    +'</div>'
...
function checkApi(){
  GET("/health").then(function(){
    ...
    toast("No DB ΓÇö running demo data","info","Demo Mode");
  });
}
